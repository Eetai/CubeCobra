        ...
        <table className="min-w-full divide-y divide-border text-sm">
          <thead className="bg-bg-accent">
            <tr>
              ...
              {renderSortHeader('Sideboard', 'deckFilterCount', 'How many pools put this card in the sideboard within the current scope')}
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {pagedRows.map((cardStatsEntry) => {
              ...
              const canToggleSideboardFilter = isFilteredSideboardCard || sideboardPoolCount > 0;
              return (
                  ...
                  <td className="px-3 py-2 text-right">
                    <button
                      ...
                      className={[
                        ...
                        !canToggleSideboardFilter ? 'opacity-40 cursor-not-allowed' : '',
                      ].join(' ')}
                      disabled={!canToggleSideboardFilter}
                      onClick={() => onSelectSideboardCard(cardStatsEntry.oracle_id)}
                    >
                      {isFilteredSideboardCard ? <>✕ <span className="tabular-nums">{sideboardPoolCount}</span></> : <span className="tabular-nums">{sideboardPoolCount}</span>}
                    </button>
                  </td>
                </tr>
              );
