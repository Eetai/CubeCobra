...
import { createCardDetails, createUser } from '../../../test-utils/data';
...
describe('Vote P1P1 API', () => {
  ...
  it('should return 400 if pack ID is missing', async () => {
    const user = createUser();
    const res = await call(voteP1P1Handler).as(user).withFlash(mockFlash).withBody({ cardIndex: 0 }).send();

    expect(res.status).toEqual(400);
    expect(res.body).toEqual({
      error: 'Pack ID and card index are required',
    });
  });

  it('should return 400 if card index is missing', async () => {
    const user = createUser();
    const res = await call(voteP1P1Handler).as(user).withFlash(mockFlash).withBody({ packId: uuid.v4() }).send();
  ...
  it('should return 400 if pack ID is invalid UUID format', async () => {
    const user = createUser();
    const res = await call(voteP1P1Handler)
      .as(user)
      .withFlash(mockFlash)
      .withBody({ packId: 'invalid-uuid', cardIndex: 0 })
      .send();
  ...
  it('should return 400 if card index is not an integer', async () => {
    const user = createUser();
    const res = await call(voteP1P1Handler)
      .as(user)
      .withFlash(mockFlash)
      .withBody({ packId: uuid.v4(), cardIndex: 'not-a-number' })
      .send();
  ...
  it('should return 400 if card index is negative', async () => {
    const user = createUser();
    const res = await call(voteP1P1Handler)
      .as(user)
      .withFlash(mockFlash)
      .withBody({ packId: uuid.v4(), cardIndex: -1 })
      .send();
  ...
  it('should return 400 if card index is out of bounds', async () => {
    const user = createUser();
    ...
    const res = await call(voteP1P1Handler)
      .as(user)
      .withFlash(mockFlash)
      .withBody({ packId: pack.id, cardIndex: pack.cards.length })
      .send();
  ...
  it('should successfully add vote and return vote summary', async () => {
    ...
    expect(p1p1PackModel.addVote).toHaveBeenCalledWith(pack.id, user.id, 'testuser', 1);
