...
const PreviousP1P1sCard: React.FC<PreviousP1P1sCardProps> = ({ packs, packsLastKey, cubeId }) => {
  ...
  // Fetch cube data once for all packs
  useEffect(() => {
    const fetchCubeData = async () => {
      try {
        const response = await fetch(`/cube/api/getcube/${cubeId}`);
        if (response.ok) {
    ...
    <Card>
      ...
      {allPacks.slice(currentPage * PAGE_SIZE, (currentPage + 1) * PAGE_SIZE).map((pack) => (
        <P1P1Preview 
          ...
          cubeOwner={cubeData?.owner?.username}
        />
      ))}
      <CardFooter>
