...
const CubeDraftPage: React.FC<CubeDraftPageProps> = ({ cube, draft, loginCallback }) => {
  ...
  const endDraft = useCallback(async () => {
    setDraftState((prev) => ({ ...prev, endDraftError: false }));
    try {
      ...
      if (response.ok) {
        ...
        throw new Error('Failed to finish draft');
      }
    } catch (err) {
      console.error('endDraft error', err);
          ...
          <DndContext onDragEnd={onMoveCard} onDragStart={() => setDragStartTime(Date.now())}>
            <div className="relative">
              <Pack
                ...
                title={packTitle}
                disabled={disabled || draftState.predictError || draftState.retryInProgress}
                ratings={ratings}
